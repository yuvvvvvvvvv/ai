...
DRAWDOWN_WARNING = 0.10  # 10% drawdown warning level
DRAWDOWN_REDUCED = 0.15  # 15% drawdown reduced trading
DRAWDOWN_STOPPED = 0.20  # 20% drawdown stop trading
...
class RiskManager:
    ...
    def check_risk_limits(self, position_size: float, entry_price: float) -> bool:
        ...
        try:
            ...
            # Check maximum drawdown if high water mark is set
            if self.high_water_mark and self.initial_equity:
                current_drawdown = 1 - (self.initial_equity / self.high_water_mark)
                if current_drawdown >= self.drawdown_levels['stopped']:
            ...
            # Check total exposure against initial equity if available
            if self.initial_equity:
                max_exposure = self.initial_equity * MAX_PORTFOLIO_EXPOSURE
            ...
            if self.market_regime == 'high_volatility':
                self.logger.warning("High volatility regime - additional risk checks applied")
                return current_exposure <= (MAX_POSITION_SIZE * entry_price * 0.5)
